from brownie import (
    network, accounts,
    config, MockV3Aggregator, VRFCoordinatorMock, Contract, LinkToken,
    interface
)
...
def get_contract(contract_name):
    ...
    otherwise it will deploy a mock version if that contract ,and return that mock
    contract.
    Args:
        contract_name (string)
    Returns:
        brownie.network.contract.ProjectContract: The most recently
         deployed of this contract
...
def fund_with_link(contract_address, account=None,
                   link_token=None, amount=100000000000000000):  # 0.1 link
